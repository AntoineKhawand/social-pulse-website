...
const stats = [
  ...
  { value: 6, suffix: " yrs", label: "In the market" },
];
  ...
  return (
    <section className="py-24 bg-dark relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 lg:px-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 md:gap-0 md:divide-x md:divide-dark-300">
          {stats.map((stat, i) => (
            <RevealBlock key={stat.label} delay={i * 0.1} className="flex flex-col md:items-center md:text-center px-0 md:px-10">
              <p className="font-display font-bold text-5xl md:text-6xl text-white mb-2">
                <Counter target={stat.value} suffix={stat.suffix} />
              </p>
              <p className="text-neutral-muted text-sm">{stat.label}</p>
            </RevealBlock>
          ))}
        </div>
      </div>
